...
def build_planning() -> dict:
    ...
    return {
        ...
        "week_1_complete": {
            ...
            "milestones": 7,
            "extra_days": ["day_4_prompts_hardening", "day_5_fastapi_ci_gitleaks", "day_6_tavily_memory"],
            "commits": [
                "b8ae5a5",
                "0d29932",
                "e549cc1",
        ...
        "day6_candidates": [
            {"item": "web_search MCP Tavily PoC", "priority": "high", "estimate": "40m", "status": "done"},
            {"item": "메모리 검색 정책 패치", "priority": "high", "estimate": "30m", "status": "done"},
            {"item": "reasoner 의도 분류 LLM 전환 검토", "priority": "medium", "estimate": "검토만", "status": "deferred"},
            {"item": "FastAPI 인증 (단순 토큰)", "priority": "medium", "estimate": "20m", "status": "done"},
            {"item": "LangSmith vs SQLite trace", "priority": "low", "estimate": "30m", "status": "deferred"},
        ],
        "day6_completed": {
            "closed": "2026-06-25",
            "items": [
                "Tavily search_web PoC + CI mock",
                "supervisor web_hits + 메모리 recall 트리거 확장",
                "GoalResponse.web_hits + output_formatter [Mem0히트]/[웹히트]",
                "FastAPI GOAL_API_TOKEN Bearer 인증",
